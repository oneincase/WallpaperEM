...
export function WallpaperTestPanel() {
  ...
  const apply = async (cfg: WallpaperConfig) => {
    ...
    setMessage(tr("已应用：{detail}", { detail: `${cfg.type}${cfg.src ? " · " + cfg.src : ""}` }));
      await refresh();
    } catch (e) {
      setMessage(String(e));
    } finally {
  ...
  return (
    <div className="card p-5">
      <div className="flex items-center justify-between mb-1">
        <h3 className="text-[15px] font-semibold">{tr("壁纸引擎原型（T0.5）")}</h3>
        <span
        ...
        >
          ...
          />
          {active ? tr("桌面壁纸运行中") : tr("未运行")}
        </span>
      </div>
      <p className="text-[12.5px] text-[var(--text-2)] mb-4">
        {tr("当前：{current} —— 验证 kCGDesktopWindowLevel 透明窗口 + WKWebView 实时渲染", {
          current,
        })}
      </p>
      ...
      <div className="flex flex-wrap gap-2 mt-3">
        <button className="btn" onClick={run("wallpaper_pause_all")}>
          <IconPause size={14} /> {tr("暂停")}
        </button>
        <button className="btn" onClick={run("wallpaper_resume_all")}>
          <IconPlay size={14} /> {tr("恢复播放")}
        </button>
        <button className="btn btn-danger" onClick={run("wallpaper_stop")}>
          <IconStop size={14} /> {tr("停止（关闭桌面窗口）")}
        </button>
      </div>

      {message && (
        <p className="mt-3 text-[12.5px] text-[var(--text-2)] break-all">{trMsg(message)}</p>
      )}
    </div>
  );
}
